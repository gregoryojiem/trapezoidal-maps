...
def handle_case2(seg, node_leaf):
    ...
    up = Node(LeafNode(Trapezoid(trap.top_seg, seg, seg.p1, seg.p2)))
    down = Node(LeafNode(Trapezoid(seg, trap.bot_seg, seg.p1, seg.p2)))
    right = Node(LeafNode(Trapezoid(trap.top_seg, trap.bot_seg, seg.p2, trap.right_vert)))
    left = Node(LeafNode(Trapezoid(trap.top_seg, trap.bot_seg, trap.left_vert, seg.p1)))
